...
class SelectHatarakikata extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    ...
    api.get('api/hataraki-kata-categories').then(response => {
      if (response.data.status === 200) {
        ...
        const chosenHatarakikata = this.props.hatarakikata.map((value, key) => {
          return {
            image: value.image,
            item_jp: value.item_jp,
  ...
  chooseHatarakikata = (id) => {
    this.setState({
      ...
      if (chosenHatarakikata.length < 4 &&
        chosenHatarakikata.filter(function(em) { return em.id === chosenCategory[id].id ? em : null })[0] === undefined) {
  ...
  submitHatarakikata = () => {
    this.props.history.push({
      pathname: '/jobs',
      state: {
        hataraki_kata: this.state.chosenHatarakikata
      }
    })
  }
            ...
            <div className="col-xs-12 col-lg-5 flex justify-content-center">
              <div className="chosen-hatarakikata-panel-container">
                ...
                <div className="submit-hatarakikata-container">
                  {chosenHatarakikata.length === 0 && <button
                    className={`btn ${(chosenHatarakikata.length === 0)? 'btn-secondary': 'btn-success'}`}
                    onClick={() => this.setState({submit: true})}>
                    { LANG[localStorage.JobChoiceLanguage].skipHataPreference }
                  </button>}
                  {chosenHatarakikata.length === 4 && <button
                    className={`btn ${(chosenHatarakikata.length === 0)? 'btn-secondary': 'btn-success'}`}
                    onClick={() => this.setState({submit: true})}>
...
export default connect(mapStateToProps, mapDispatchToProps)(ReactTimeout(SelectHatarakikata))
