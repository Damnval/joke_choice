...
class JobWelfareModal extends Component {
    ...
    handleClose() {
      this.props.onClose({welfare: false})
      this.setState({
        show: false
      })
    }
            ...
            <ReactModal
                ...
                <div className='dialog__content' ref='content'>
                    ...
                    <div className={`educational-modal-header`}>
                      <span>{LANG[localStorage.JobChoiceLanguage].welfare}</span>
                    </div>
                    <div className='modal-body'>
                      {this.state.details.length > 0 ? this.state.details.map((value, key) => {
                        return (
                          <div className="career-row" key={key}>
                            {EM[localStorage.JobChoiceLanguage].WELFARE.filter(el => el.value === value.name)[0].name}
                          </div>
                        )
                      }):
