...
class ReasonToHireModal extends Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    handleClose() {
      this.props.onClose({reasonToHire: false})
      this.setState({
        show: false
      })
    }
            ...
            <ReactModal
                ...
                <div className='dialog__content' ref='content'>
                    ...
                    <div className={`educational-modal-header`}>
                      <span>{LANG[localStorage.JobChoiceLanguage].reasonsToHire}</span>
                    </div>
                    <div className='modal-body'>
                      {this.state.details.length > 0 ? this.state.details.map((value, key) => {
                        return (
                          <div className="career-row" key={key}>{value.reason}</div>
                        )
                      }):
...
export default connect(mapStateToProps, mapDispatchToProps)(ReasonToHireModal)
