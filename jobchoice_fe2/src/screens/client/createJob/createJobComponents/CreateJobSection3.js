...
class CreateJobSection3 extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    if(this.props.viewToEdit === true) {
      if(this.props.viewToEditData.job_strengths && this.props.viewToEditData.job_strengths.length > 0) {
        var id_new = 0
        const job_strengths = this.props.viewToEditData.job_strengths.map((value, key) => {
          return ({
            id: id_new + key,
            item: value.item,
        ...
        const formErrors = this.props.viewToEditData.job_strengths.map((value, key) => {
          return ({
...
export default connect(mapStateToProps)(CreateJobSection3)
