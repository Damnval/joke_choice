...
const formValid = ({ formErrors, section1 }) => {
  ...
  Object.values(formErrors).forEach(val => {
    ...
    if(val && val !== undefined && val !== null && typeof val === 'object'){
      Object.values(val).forEach(geolocation => {
        geolocation.length > 0 && (valid = false)
      })
    }
  })
  ...
  Object.values(section1).forEach(val => {
    if(val && val !== undefined && val !== null && typeof val === 'object'){
      Object.values(val).forEach(geolocation => {
        (!geolocation || geolocation === null || (typeof geolocation === 'string' && geolocation.trim().length === 0)) && (valid = false)
      })
    } else {
      (!val || val === null ||
...
class CreateJobSection1 extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    if(this.props.viewToEdit === true) {
      const employment_period = this.props.viewToEditData.employment_period ? this.props.viewToEditData.employment_period : null
      this.setState({
        section1: {
          employment_period: this.props.viewToEditData.employment_period,
          employment_type: this.props.viewToEditData.employment_type,
          service_company: this.props.viewToEditData.service_company,
          geolocation: {
            zip_code: this.props.viewToEditData.geolocation && this.props.viewToEditData.geolocation.zip_code !== null ? this.props.viewToEditData.geolocation.zip_code : '',
            complete_address: this.props.viewToEditData.geolocation && this.props.viewToEditData.geolocation.complete_address !== null ? this.props.viewToEditData.geolocation.complete_address : '',
            lat: this.props.viewToEditData.geolocation && this.props.viewToEditData.geolocation.lat !== null ? this.props.viewToEditData.geolocation.lat : 10.000,
            lng: this.props.viewToEditData.geolocation && this.props.viewToEditData.geolocation.lng !== null ? this.props.viewToEditData.geolocation.lng : 10.000,
            prefectures: this.props.viewToEditData.geolocation && this.props.viewToEditData.geolocation.prefectures !== null ? this.props.viewToEditData.geolocation.prefectures : '',
          },
          incentive_per_share: this.props.viewToEditData.incentive_per_share,
          price: this.props.viewToEditData.price,
        },
        optional: {
          location_details: this.props.viewToEditData.location_details,
        },
        lowestValue: employment_period !== null ? EM[localStorage.JobChoiceLanguage].EMPLOYMENT_PERIOD.filter(function (item) {return item.value === employment_period})[0].price : '',
      })
    }
  }
  ...
  copyCompany = e => {
    ...
    this.setState({
      section1: {
        ...this.state.section1,
        service_company: this.props.company.company_name,
        geolocation: {
          ...this.state.section1.geolocation,
          zip_code: this.props.company.geolocation && this.props.company.geolocation.zip_code !== null ? this.props.company.geolocation.zip_code : null,
          complete_address: this.props.company.geolocation && this.props.company.geolocation.complete_address !== null ? this.props.company.geolocation.complete_address : null,
          lat: this.props.company.geolocation && this.props.company.geolocation.lat !== null ? this.props.company.geolocation.lat : null,
          lng: this.props.company.geolocation && this.props.company.geolocation.lng !== null ? this.props.company.geolocation.lng : null,
          prefectures: this.props.company.geolocation && this.props.company.geolocation.prefectures !== null ? this.props.company.geolocation.prefectures : null,
        },
      },
      formErrors: {
  ...
  handleFormError(name, value) {
    ...
    switch (name) {
      ...
      case "price":
        if(this.state.lowestValue !== null) {
          formError = value !== null && parseFloat(String(value).replace(/,/g, '')) < this.state.lowestValue ? LANG[localStorage.JobChoiceLanguage].valueMustNotLower1 + this.state.lowestValue + LANG[localStorage.JobChoiceLanguage].valueMustNotLower2  : value && value !== null && value.length > 6 ? LANG[localStorage.JobChoiceLanguage].incentivePerShare + LANG[localStorage.JobChoiceLanguage].maxCharacters1 + '5' + LANG[localStorage.JobChoiceLanguage].maxCharacters2 : ""
        } else {
          formError = value.length > 6 ? LANG[localStorage.JobChoiceLanguage].incentivePerShare + LANG[localStorage.JobChoiceLanguage].maxCharacters1 + '5' + LANG[localStorage.JobChoiceLanguage].maxCharacters2 : ""
        }
      break
      case "employment_period":
        if(this.state.lowestValue !== null) {
          formError = value < this.state.lowestValue ? LANG[localStorage.JobChoiceLanguage].valueMustNotLower1 + this.state.lowestValue + LANG[localStorage.JobChoiceLanguage].valueMustNotLower2  : ""
        } else {
          formError = ""
        }
      break
    ...
    return (
      <div className="createJob-section-bg">
        ...
        <div className="createJob-input-links">
          <div></div>
          <div>
            <a href="#" onClick={()=>this.handleInDevelopment()} className="job-offer-detail-link-share">※{LANG[localStorage.JobChoiceLanguage].pleaseReferHereShareReward}</a>
            <a href="#" onClick={()=>this.handleInDevelopment()} className="job-offer-detail-link-share">※{LANG[localStorage.JobChoiceLanguage].pleaseReferMoneyPerShare}</a>
          </div>
        </div>
...
export default connect(mapStateToProps)(CreateJobSection1)
